...
def retrain():
    save_dir_ginit = "samples/{}_ginit_retrain".format(tl.global_flag['mode'])
    save_dir_gan = "samples/{}_gan_retrain".format(tl.global_flag['mode'])
    tl.files.exists_or_mkdir(save_dir_ginit)
    tl.files.exists_or_mkdir(save_dir_gan)
    checkpoint_dir = "checkpoint"  # checkpoint_resize_conv
    tl.files.exists_or_mkdir(checkpoint_dir)
    ...
    #net_vgg, vgg_target_emb = Vgg19_simple_api((t_target_image_224 + 1) / 2, reuse=False)
    #_, vgg_predict_emb = Vgg19_simple_api((t_predict_image_224 + 1) / 2, reuse=True)
    disc_target_emb = SRGAN_d(t_target_image)
    disc_predict_emb = SRGAN_d(t_predict_image)

    net_g = SRGAN_g(t_image, is_train=True, reuse=False)
    net_d, logits_real = SRGAN_d_retrain(t_target_image, is_train=True, reuse=False)
    _, logits_fake = SRGAN_d_retrain(net_g.outputs, is_train=True, reuse=True)
    ...
    g_gan_loss = 1e-3 * tl.cost.sigmoid_cross_entropy(logits_fake, tf.ones_like(logits_fake), name='g')
    mse_loss = tl.cost.mean_squared_error(net_g.outputs, t_target_image, is_mean=True)
    vgg_loss = 2e-6 * tl.cost.mean_squared_error(disc_predict_emb, disc_target_emb, is_mean=True)
